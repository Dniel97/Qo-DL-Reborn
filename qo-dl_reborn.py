...
def parse_prefs(cfg, tag_cfg):
    if cfg:
        ...
        rt_opts = {
            ...
            "track_format": cfg['prefs']['track_format'],
            "album_format": cfg['prefs']['album_format']}
        tags = {
...
def download(id, album_fol_s, meta, num, tot, fol, qual, cov, ref, tag_cfg, track_format, cov_dir, embed_cov):
    if not track_format.strip():
        track_format = str(num) + ". " + meta['TITLE']
    else:
        track_format = track_format.upper().format(**meta)
    title = meta['TITLE']
    if not tot:
        tot = meta['TRACKTOTAL']
    ...
    if url_resp['format_id'] == 5:
        ...
        ext = ".flac"
    post = os.path.join(album_fol_s, sanitize(track_format) + ext)
    exist_check(pre)
    if exist_check(post):
...
def main(type, id, cfg, tag_cfg, fol, cli):
    ...
    track_format = cfg['track_format']
    album_format = cfg['album_format']
    ...
    while True:
        ...
        if type == "album":
            ...
            if not album_format.strip():
                album_fol = al_meta['ALBUMARTIST'] + " - " + al_meta['ALBUM']
            else:
                album_fol = album_format.upper().format(**al_meta)
            ...
            for track in src_meta['tracks']['items']:
                ...
                download(track['id'], album_fol_s, final_meta, num, None, fol, qual, cov, ref, tag_cfg, track_format,
                         cov_dir, embed_cov)
            ...
            download(src_meta['id'], album_fol_s, final_meta, 1, 1, fol, qual, cov, ref, tag_cfg, track_format, cov_dir,
                     embed_cov)
            ...
            for dict in src_meta:
                ...
                for track in tracks:
                    ...
                    if not download_cov(cov, cov_dir):
                        cov_dir = None
                    final_meta = parse_meta(track, al_meta, num, None)
                    download(track['id'], album_fol_s, final_meta, num, tot, fol, qual, cov, ref, tag_cfg, track_format,
                             cov_dir, embed_cov)
            os.remove(cov_dir)
